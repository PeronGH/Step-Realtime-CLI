...
Step Code 在设计阶段参考了 **OpenAI Codex CLI**、**Claude Code**、**OpenCode**、**某竞品CLI** 等优秀项目的架构思路与交互设计（计划模式、技能懒加载、自主目标等尤其受 Codex 与 Claude Code 启发）。本项目源码自行编写，与上述项目无隶属、赞助或背书关系；相关开源许可证收录于 [`licenses/`](./licenses/) 目录，详见 [`licenses/NOTICE.md`](./licenses/NOTICE.md)。
