...
describe('loadConfig [models.<别名>] 集成', () => {
  ...
  it('别名缺省 model 字段 → 真实 id = 别名本身', () => {
    writeToml(['api_key = "k"', 'model = "step-3.5-flash"', '', '[models.step-3.5-flash]', ''].join('\n'));
    const cfg = loadConfig(dir);
  ...
  it('--model 别名（overrides）同样展开', () => {
    writeToml(
      ['api_key = "k"', '', '[models.fast]', 'model = "step-3.7-flash"', 'max_tokens = 8192', ''].join('\n'),
    );
  ...
  it('model 未命中别名 → 原样保留，不受影响', () => {
    writeToml(['api_key = "k"', 'model = "plain-model"', '', '[models.fast]', 'model = "test-model-x"', ''].join('\n'));
    const cfg = loadConfig(dir);
    expect(cfg.model).toBe('plain-model');
  });

  it('[models] 全部无效 → models 键不进结果对象', () => {
    writeToml(['api_key = "k"', '', '[models]', ''].join('\n'));
    const cfg = loadConfig(dir);
