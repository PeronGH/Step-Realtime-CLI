...
function historyWithFacts(): StoredMessage[] {
  return [
    stored({ role: 'user', content: '项目在 C:/Users/ke/Documents/projects/CodeProjects/Windows-Native/step-code-suite' }, 'user'),
    bulkAssistant('A1'),
...
describe('fullCompact 摘要质量闸门', () => {
  ...
  it('重试收缩掉最老的消息，但用户原话仍由保真块兜住', async () => {
    ...
    const out = await fullCompact(provider, historyWithFacts(), 2);
    const verbatim = out.filter((m) => m.origin === 'user_verbatim').map((m) => String(m.message.content));
