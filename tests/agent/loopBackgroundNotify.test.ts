...
describe('runAgent 后台通知 step 边界注入', () => {
  it('busy 中终态：通知在下一回合前注入（第 2 次模型调用已可见），不等循环结束', async () => {
    ...
    const events = await collect(runAgent(runOpts(provider, messages, background, true)));

    // 第 1 次调用（回合 1）尚无通知；第 2 次调用（回合 2）已带通知 → 注入早于循环结束
    expect(callMessagesText(streamParams(), 0)).not.toContain('background_task');
    expect(callMessagesText(streamParams(), 1)).toContain('[background_task');
    expect(callMessagesText(streamParams(), 1)).toContain('npm test');
    expect(events.at(-1)!.type).toBe('turn_done');
    // 通知作为独立 user 条目进入会话历史
    const note = messages.find((m) => typeof m.message.content === 'string' && m.message.content.includes('[background_task'));
    expect(note).toBeDefined();
    expect(note!.origin).toBe('user');
    // 已 drain，不留残余
  ...
  it('多条同时终态：同批注入、各自独立条目（不合成单条大消息）', async () => {
    ...
    const notes = messages.filter(
      (m) => typeof m.message.content === 'string' && m.message.content.includes('[background_task'),
    );
  ...
  it('未开启开关（如 -p 模式）：不在循环内注入，通知留在管理器待投递队列', async () => {
    ...
    expect(callMessagesText(streamParams(), 1)).not.toContain('background_task');
