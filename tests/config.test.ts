...
import { loadConfig, resolveCompactionConfig, resolveModelEntry, resolveModels, resolveProviders, resolveStringArray, resolveSubagentLimits, type StepCodeConfig } from '../src/config/config.js';
...
describe('loadConfig', () => {
  ...
  it('有 key 时给出合理默认（model=step-3.7-flash）', () => {
    if (existsSync(join(homedir(), '.step-code', 'config.toml'))) return;
  ...
  it('完全无 key 时抛出明确错误', () => {
    if (existsSync(join(homedir(), '.step-code', 'config.toml'))) return;
    expect(() => loadConfig(dir)).toThrow(/API key/);
  });
...
describe('resolveModelEntry（自定义渠道合并）', () => {
  ...
  it('渠道缺省字段回落 entry → 顶层', () => {
    ...
    // apiKey：渠道与 entry 都缺省 → 顶层
    expect(merged2!.apiKey).toBe('k-top');
  });
