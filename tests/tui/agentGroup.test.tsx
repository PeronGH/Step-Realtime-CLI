...
describe('AgentGroup 面板', () => {
  ...
  it('行格式：tools · 时长 · tok 三段（分钟级带秒、千进制 tok）', () => {
    // startedAt 留 500ms 余量：渲染延迟不跨秒界
    const { lastFrame } = render(
      React.createElement(AgentGroup, {
        agents: [
          { id: '1', type: 'explore', description: '统计代码结构', status: 'running', toolCount: 14, startedAt: Date.now() - 148_500, tokens: 107_000 },
        ],
      }),
    );
  ...
  it('tokens 为 0 / 缺省不显示 tok 段', () => {
    const { lastFrame } = render(
      React.createElement(AgentGroup, {
        agents: [
          { id: '1', type: 'explore', description: 'a', status: 'running', toolCount: 3, startedAt: Date.now() - 5_500, tokens: 0 },
          { id: '2', type: 'explore', description: 'b', status: 'running', toolCount: 1, startedAt: Date.now() - 5_500 },
        ],
      }),
    );
    const out = lastFrame() ?? '';
