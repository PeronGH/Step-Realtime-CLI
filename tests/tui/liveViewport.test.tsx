...
describe('MessageList 尾部锚定窗口（maxRows）', () => {
  ...
  it('单 item 超长：行级截尾（不是整 item 丢弃）+ 隐藏行数指示', async () => {
    ...
    const { lastFrame } = render(<MessageList items={[assistant(text)]} expanded={false} maxRows={6} />);
    await settle();
    const out = lastFrame() ?? '';
  ...
  it('多 item 超预算：从尾部累计裁剪，顶部 item 先被隐藏', async () => {
    ...
    const { lastFrame } = render(<MessageList items={items} expanded={false} maxRows={8} />);
    await settle();
    const out = lastFrame() ?? '';
    const lines = frameLines(out);
  ...
  it('不超预算时原样渲染：无指示、内容完整、帧高不变', async () => {
    ...
    const windowed = render(<MessageList items={items} expanded={false} maxRows={10} />);
    await settle();
    ...
    const plain = render(<MessageList items={items} expanded={false} />);
  ...
  it('不传 maxRows：长内容原样渲染，无窗口化（回归保护）', async () => {
    ...
    const { lastFrame } = render(<MessageList items={[assistant(text)]} expanded={false} />);
    await settle();
    const out = lastFrame() ?? '';
    expect(out).not.toContain('已隐藏');
  ...
  it('预算收紧（模拟 resize）：窗口收缩，隐藏行数指示随之变大', async () => {
    ...
    const { lastFrame, rerender } = render(
      <MessageList items={[assistant(text)]} expanded={false} maxRows={15} />,
    );
    await settle();
    // 自然 21 行 > 15 → 指示 1 + 可见 14，隐藏 7
    expect(lastFrame() ?? '').toContain('已隐藏 7 行早期输出');
    rerender(<MessageList items={[assistant(text)]} expanded={false} maxRows={6} />);
    await settle();
    const out = lastFrame() ?? '';
    expect(out).toContain('已隐藏 16 行早期输出');
  ...
  it('集成：长流式 assistant + Ctrl+O 展开长工具输出，动态帧高恒 ≤ 预算', async () => {
    ...
    // 自然高度 = assistant 16 + tool 52（margin 1 + 头 1 + 输出 50）= 68；maxRows 8 → 指示 1 + 可见 7
    const { lastFrame } = render(<MessageList items={items} expanded={true} busy={true} maxRows={8} />);
    await settle();
    const out = lastFrame() ?? '';
    const lines = frameLines(out);
    expect(out).toContain('已隐藏 61 行早期输出');
    // 尾部锚定：工具输出末尾可见，流式开头与工具头部被裁（ResultBody marginLeft=2，行带两格前导空格）
    expect(lines).toContain('  r50');
    expect(lines).not.toContain('  r43');
