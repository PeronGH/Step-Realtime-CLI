...
- **禁止**：`CHANGELOG` / 用户文档正文 / 源码注释 / commit message 里出现「与某竞品 CLI 一致」「对齐某某」「借鉴某某」「移植自某某」等横向对标或来源归因表述。功能就按它本身的价值来描述，说清行为、动机、用法即可。
- **允许且应当**：
...
- **范围**：本规范约束源码仓库所有可追踪内容（代码、注释、文档、提交信息）。竞品研究与横向对标只留在独立的产品设计仓库，不进入本源码仓库。
- **设计文档不进本仓库**：产品/交互/技术设计稿（PRD、方案、取舍记录、竞品调研等）一律放到独立的产品设计仓库（`step-code-product-design`），**不要放进本源码仓库**（含 `docs/design/` 这类路径）。本仓库的 `docs/` 只保留面向用户的使用文档。设计稿属于内部材料，可自由横向对标，但不应随源码对外发布。
