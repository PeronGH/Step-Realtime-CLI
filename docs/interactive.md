...
| Alt+V | 从剪贴板粘贴图片：在输入框光标处插入占位符 `[image #1 (宽×高)]`，可像普通文字一样编辑删除，提交时展开为图片。Windows 用系统自带 PowerShell；macOS 用系统自带 osascript；Linux 需装 `xclip`（X11）或 `wl-clipboard`（Wayland），未装时会提示 |
...
`/resume` 无参（或命令行 `step -r` 不带 id）打开交互式会话选择器：↑↓ 移动（分页翻页看完全部会话）、直接打字模糊搜索、Enter 恢复、Esc 取消，Delete/Ctrl+D 删除高亮会话（`[y/N]` 二次确认，当前会话不可删）。`/resume <id>` 文本直切、`/sessions` 只读列出。权限模式与模型随会话持久化，恢复时读回。详见[会话管理](./sessions.md#交互式会话选择器命令行--r-与界面内-resume-通用)。
...
- **无可见思考文本时**：有些模型只回思考签名、不回思考正文（思考块存在但内容为空）。此时状态行仍显示「思考中…」（只有标题、没有预览行），从思考块开始持续到思考块结束——用来区分「模型在想」与「卡住了」。
- **完成后**：落一个暗色斜体折叠块，最多显示前 5 行，超出折叠为「…（共 N 行）」。

忙碌态状态行（spinner 那行）的随机状态词是「处理中/推进中/…」这类中性词，不声称思考——是否在思考只由上面的「思考中…」表达。
