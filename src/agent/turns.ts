 ...
 * role:user / origin:tool 的消息。这个「条数」是上下文/压缩/token 预算的真实尺寸，
 * 但对用户没有认知意义——用户想的是「我们聊了几个来回」。
 *
 * 轮次即从用户认知出发的派生单位：一轮 = 一次真人输入（origin:'user'）起，
...
function isUserTurnStart(m: StoredMessage): boolean {
  // origin:'user' 覆盖真人输入与后台任务通知等以 user 身份注入的起始消息；
  // origin:'tool'（tool_result 回灌）虽然 role 也是 user，但不算新一轮的起点。
  return m.origin === 'user';
}
