...
/** system → 带 cache_control 的 TextBlock 数组。 */
export function buildSystemBlocks(system: string): Anthropic.TextBlockParam[] {
  return [{ type: 'text', text: system, cache_control: CACHE_CONTROL }];
}

/** 给最后一个 tool 定义打 cache_control（缓存整段 tool 定义前缀）。 */
export function withToolCacheControl(tools: Anthropic.Tool[]): Anthropic.Tool[] {
 ...
 * 3. 在最后一条消息的最后一个 content block 注入 cache_control。
 * 不改动入参数组本身（返回浅拷贝）。
 */
export function prepareMessages(input: Anthropic.MessageParam[]): Anthropic.MessageParam[] {
...
function injectCacheControlOnLastBlock(messages: Anthropic.MessageParam[]): void {
